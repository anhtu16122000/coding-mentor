...
// import { userInformationApi } from '~/src/apiBase'
...
const StaffSalaryForm = (props) => {
	...
	const onSubmit = handleSubmit((data: any) => {
		...
		if (props.dataStaff && props.dataStaff.length > 0) {
			...
			res.then(function (rs: any) {
				if (rs && rs.status == 200) {
					setIsModalVisible(false), form.resetFields(), setParamsDataStaff(null)
				}
			})
		} else {
	...
	const getSearchDataStaff = (name) => {
		...
		;(async () => {
			try {
				let res = await userInformationApi.getName({ pageIndex: 1, pageSize: PAGE_SIZE, FullNameUnicode: name })
