...
function ExamGroups(props) {
	...
	async function getData() {
		try {
			const res = await ieltsGroupApi.getAll({ pageSize: 99, pageIndex: 1, ieltsSkillId: data?.Id })
			if (res.status == 200) {
	...
	async function saveChangedSections(params) {
		try {
			const res = await ieltsGroupApi.changeIndex(params)
			if (res.status == 200) {
	...
	return (
		<div className="cc-exam-detail !max-w-[1200px]">
			<div className="mb-[16px] flex items-center">
				...
				{showEditIndex && changed && (
					<PrimaryButton onClick={getData} className="mr-[8px]" background="yellow" icon="reset" type="button">
						Khôi phục
					</PrimaryButton>
				)}
