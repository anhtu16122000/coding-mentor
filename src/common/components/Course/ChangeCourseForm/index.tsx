	...
	const onSubmit = async (data: any) => {
		...
		if (infoId) {
			if (courseAfterDetail?.Price > infoDetail?.Price) {
				ShowNoti('error', 'Không được chuyển qua khóa rẻ hơn')
			} else {
				setLoading(true)
				try {
			...
			<Modal title="Chuyển khóa" visible={isModalVisible} onCancel={() => setIsModalVisible(false)} footer={null}>
				<div className="container-fluid">
					<Form form={form} layout="vertical" onFinish={onSubmit}>
						...
						<Spin spinning={isLoading.status}>
							<div className="row">
							...
							{courseAfter != null && (
								<div className="row">
									<div className="col-12">
										<Form.Item name="CourseIDAfter" label="Lớp học chuyển đến">
											<Select
												style={{ width: '100%' }}
												className="style-input"
												onChange={handleChangeCourseAfter}
												placeholder="Chọn lớp học chuyển đến"
											>
