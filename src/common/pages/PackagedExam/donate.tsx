...
import { packageStudentApi } from '~/api/packed/package-student'
...
const DonatePackage: FC<IDonateVideo> = ({ item, onRefresh }) => {
	...
	const _active = async (code) => {
		setLoading(true)
		try {
			let res = await packageStudentApi.post({ PackageId: item.Id, StudentId: code })
			if (res.status == 200) {
				!!onRefresh && onRefresh()
				setVisible(false)
	...
	return (
		<>
			<div onClick={toggle} className="pe-i-d-gift">
				<FaGift size={14} />
			...
			>
				...
				<Form
					form={form}
					className="grid grid-cols-2 gap-x-4 ant-chaos-custom"
					layout="vertical"
				...
				>
					...
					{!!textError && <div className="text-[red] mt-[-16px]">{textError}</div>}
				</Form>
			</Modal>
		</>
	)
