...
import NotificationForm from '~/common/components/Notification/NotificationForm'
...
const Notification = () => {
	...
	const getDataTable = async () => {
		...
		try {
			let res = await notificationApi.getAllSend(todoApi)
			if (res.status == 204) {
	...
	return (
		<ExpandTable
			...
			// }}
			expandable={(record) => <p style={{ margin: 0 }}>{ReactHtmlParser(record.NotificationContent)}</p>}
			TitleCard={
